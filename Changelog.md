...
- Allow Portraits of Actors and Items to be edited by their owner.
- Ensure that an All-Out attack will give the attacker the Very Vulnerable effect.
- New game system option to suppress chat messages generated by core Foundry where TE provides its own message.
- The links in the Welcome Message now work again (opening the relevant compendium).
- Allow Welcom message to be shown at any time by calling `game.torgeternity.showWelcomeMessage()`
