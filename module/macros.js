export class torgeternityMacros {
    async ClearStatusEffects(){
        ...
        for (const token of tokens) {
            let effects = token.actor.statuses;
            ...
            for (const effect of effects) {
                switch (effect) {
                    ...
                    case "veryVulnerable":
                        ...
                        chatOutput += `<li>${game.i18n.localize("torgeternity.macros.clearStatusEffectsliftetFrom")} ${token.actor.name} ${game.i18n.localize("torgeternity.macros.clearStatusEffectsliftetStatusOf")} ${game.i18n.localize("torgeternity.statusEffects." + effect)}</li>`;
                        break;
                    default:
                        continue;
                }            
        ...
        ChatMessage.create({content: chatOutput});} catch (e) {
    ...
    async ApplyFatigue() {
        ...
        let chatOutput = `<h2>${game.i18n.localize("torgeternity.sheetLabels.fatigue")}!</h2><p>${game.i18n.localize("torgeternity.macros.fatigueMacroDealtDamage")}</p><ul>`
        for(const _target of tokens){
            ...
            if (_target.actor.system.shock.value === _target.actor.system.shock.max) {
                chatOutput += `<li>${_target.actor.name} ${game.i18n.localize("torgeternity.macros.fatigueMacroCharAlreadyKO")}</li>` 
                continue;
            }
            
            let targetShockValue = _target.actor.system.shock.value;
            
            let shockIncrease = _target.actor.system.other.fatigue;
            ...
            console.log(`Successfully increased the shock for ${_target.document.name} for the amount of ${shockIncrease} to ${shockResult}.`);
            chatOutput += `<li>${_target.document.name}: ${shockIncrease} ${game.i18n.localize("torgeternity.sheetLabels.shock")}`
            if (parseInt(_target.actor.system.shock.value) >= parseInt(_target.actor.system.shock.max) && !_target.actor.statuses.find((d) => d === "unconscious")){ 
                const eff = CONFIG.statusEffects.find((e) => e.id === "unconscious");
                _target.toggleEffect(eff, {active: true, overlay: true});
                chatOutput += `<br><strong>${_target.document.name}${game.i18n.localize("torgeternity.macros.fatigueMacroCharKO")}</strong>`
            }
            chatOutput += "</li>"
        }    
        chatOutput += "</ul>"
    ...
    async ReviveShock(){
        const windowContent = `
            <form style="margin-bottom: 1rem; display:grid; grid-template-rows: repeat(2,auto); grid-template-columns: repeat(2,auto); align-items: center; gap:6px;">
                ...
                <input style="grid-row: 2/3;grid-column: 2/3;" type="checkbox" name="wholeRevive"> <label for="wholeRevive" style="grid-row: 2/3; grid-column: 1/2">${game.i18n.localize("torgeternity.macros.reviveMacroWholeRevive")}
            </form>
        `
    ...
    async _processReviveShock(html){
        try {
            ...
            let chatOutput = `<h2>${game.i18n.localize("torgeternity.macros.reviveMacroChatHeadline")}</h2><p>${game.i18n.localize("torgeternity.macros.reviveMacroFirst")}<p><ul>`
            ...
            for (const _target of tokens) {
                ...
                if (targetShockValue === 0) {
                    chatOutput += `<li>${_target.actor.name} ${game.i18n.localize("torgeternity.macros.reviveMacroAlreadyFull")}</li>`
                    continue;
                }
                ...
                if (bolWholeRevive) {
                    ...
                    chatOutput += `<li>${_target.actor.name} ${game.i18n.localize("torgeternity.macros.reviveMacroCharPartyRevived")}${reviveAmount}`;
                }
